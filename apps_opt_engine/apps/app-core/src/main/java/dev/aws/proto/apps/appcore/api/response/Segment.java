...
public class Segment {
    ...
    public static Segment fromSegments(List<DeliverySegment> segments) {
        ...
        for (DeliverySegment segment : segments) {
            encodedPolylines.add(segment.getRoute().pointsEncoded);

            allDistInMeters += segment.getRoute().getDistance().getValue();
            allDistTimeInSec += segment.getRoute().getTime().getValue();
        }
