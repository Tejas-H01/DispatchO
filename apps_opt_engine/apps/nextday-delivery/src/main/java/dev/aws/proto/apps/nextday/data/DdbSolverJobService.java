...
public class DdbSolverJobService extends DdbServiceBase {
    ...
    public DdbSolverJobService(DdbProperties ddbProperties) {
    ...
    protected Map<String, AttributeValue> getPutItemMap(Object solverJob_) {
        ...
        if (solverJob.getWarehouseName() != null)
            item.put("warehouseName", AttributeValue.builder().s(solverJob.getWarehouseName()).build());
        item.put("orderDate", AttributeValue.builder().s(solverJob.getOrderDate()).build());
        if (solverJob.getOrderCount() >= 0)
            item.put("orderCount", AttributeValue.builder().s(String.valueOf(solverJob.getOrderCount())).build());
