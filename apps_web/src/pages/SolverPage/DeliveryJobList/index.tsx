...
import { OptimizationSummaryDashboard } from './OptimizationSummaryDashboard'
...
export const DeliveryJobList: FunctionComponent = () => {
  ...
  const [solverJob, setSolverJob] = useState<SolverJobData | null>(null)

  const fetchSolverJob = async () => {
    if (!solverJobId) return
    try {
      setLoading(true)
      const res = await NextDayDelivery.getSolverJobById(solverJobId)
      setSolverJob(res?.data?.Item ?? null)
    } catch (err) {
      console.log(err)
    } finally {
      setLoading(false)
  ...
  return (
    <SpaceBetween size='l'>
      <OptimizationSummaryDashboard
        deliveryJobs={deliveryJobs}
        solverJob={solverJob}
        loading={loading}
      />
      <ConstraintWarnings solverJob={solverJob} loading={loading} />
      <Container>
