...
export class CrudService<TData> implements IService<TData> {
  ...
  async create(data: TData): Promise<TData> {
    try {
      const response = await common.commonPostRequest(`/${this.endpoint}`, data)
      const {
        data: { Item },
      } = response
  ...
  async update(data: TData): Promise<TData> {
    try {
      const response = await common.commonPutRequest(`/${this.endpoint}`, data)
      const {
        data: { Item },
      } = response
  ...
  async deleteItem(id: IdType): Promise<void> {
    try {
      const response = await common.commonDeleteRequest(`/${this.endpoint}/${id}`)
      console.debug(`[api::${this.serviceName}::delete] Successfully deleted with id ${id}`, response?.data)
    } catch (err) {
