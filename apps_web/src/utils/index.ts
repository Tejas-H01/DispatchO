...
const sortByCreationDateDesc = <T extends { createdAt: number }>(arr: T[]): T[] => {
...
const calculateDistanceByZoom = (zoom: number): number => {
  ...
  let pixelMeter: number = pixelMeterSizes[8]

  if (zoom > 20) {
    pixelMeter = pixelMeterSizes[20]
  } else if (zoom > 8 && zoom in pixelMeterSizes) {
...
const generatePolygonGEOJSON = (vertices: ILatLong[]): any => {
  return {
    type: 'Feature',
    properties: {},
    geometry: {
...
const getDefaultAreas = (): ILatLong[] => {
  const areas = (appVariables?.DEFAULT_AREAS as ILatLong[] | undefined) ?? []
  return areas
}

const getAreaCodeFromCoords = (lat: number, long: number): string | undefined => {
  const areas = getDefaultAreas()
  const idx = areas.findIndex((q) => q.lat === lat && q.long === long)
